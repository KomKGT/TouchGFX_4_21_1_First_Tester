...
*/
  ...
  /*HAL_UART_Receive_IT(&huart3, (uint8_t*) ch_readBuf, 256);
  HAL_UART_Receive_IT(&huart2, (uint8_t*) ch_readBuf, 256);*/
...
//	HAL_UART_Receive_IT(&huart2, (uint8_t *)ch_readBuf, 10);
...
//	data = "";
	...
	//HAL_UART_Receive(&huart2, (uint8_t*)ch_readBuf, 2, 1000);
	for(int i=1;i<100;i++)
	{
		data = data + ch_readBuf[i];
		if(ch_readBuf[i]=='\r')
		{
			ch_readBuf[++i] = '\0';
			break;
